 ...
 *
 * 【実データで確認できたこと】
 * - 「コースレコード／中央レコード」直後に並ぶ馬場状態表記は、そのレースの実況では
 *   なく、該当コース・距離・サーフェスの歴代レコード（コースレコード／JRAレコード）が
 *   成立した時点の馬場状態という静的な値だった。同じコース・距離・サーフェスの
 *   レースが日をまたいでも常に同じ値・同じタイムを示すことで判明（例:
 *   2025年4月12日福島1700mダートの第1競走・第7競走が、実施時刻が4時間近く
 *   離れているにもかかわらず「1：42．6／1：40．9／稍重／重」で完全一致）。
 *   このため馬場状態には使わない。
 * - ヘッダー行内、天候直後にある「日付 天候 馬場状態 （開催情報）」の馬場状態こそが、
 *   そのレース自身の実況値。発走時刻・サーフェスごとに正しく変化することを確認済み
 *   （例: 2025年5月17日東京は雨で芝が「良→稍重」、ダートが「稍重→不良」に変化し、
 *   これが個々のレースの発走順・サーフェスと矛盾なく対応していた。日次サマリーの
 *   「馬場（芝Bコース）良後稍重／（ダート）稍重後不良」とも整合）。
...
export function parseDayResultText(rawText: string, diagnostics?: ParseDiagnostics): ParsedRaceResult[] {
  ...
  for (const block of blocks) {
    ...
    // 馬場状態：ヘッダー行の「日付 天候 馬場状態 （開催情報）」から取得。
    // 「中央レコード」直後の値は、そのレースの実況ではなく該当コース・距離・サーフェスの
    // 歴代レコード成立時の馬場状態という静的な値だったため使わない（詳細は本ファイル冒頭の
    // コメント参照）。ヘッダーの値は実際に発走時刻・サーフェスごとに変化することを
    // 2025年4月12日福島(終日「良」)・2025年5月17日東京(雨で芝「良→稍重」、
    // ダート「稍重→不良」)の実データで確認済み。
    let condition = '不明';
    const headerCondMatch = block.match(/^\d{5}\s*\d{1,2}月\s*\d{1,2}日\s*\S+\s*(良|稍重|重|不良)\s*[（(]/);
    if (headerCondMatch) condition = headerCondMatch[1];
