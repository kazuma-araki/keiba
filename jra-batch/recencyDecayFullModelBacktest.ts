/**
 * recencyDecayBacktest.tsは「タイム指数のみ」の古いモデルに対する検証だった。
 * その後、騎手係数・距離変更・間隔(alphaInterval=0.1, alphaDistance=0.05)が本番に
 * 追加されたため、tau=60日の指数減衰がまだ有効かを、現行のフルモデル
 * (タイム指数×騎手係数×距離変更×間隔)の上で再検証する。
 *
 * finalScore = timeScore(tau) + ALPHA_JOCKEY*jockeyZ + ALPHA_INTERVAL*intervalZ - ALPHA_DISTANCE*distanceDeltaZ
 * ここでtimeScore(tau)の「新しさの重み」だけを、現行の順序ベース[1.0,0.8,0.6,0.4]と
 * 指数減衰exp(-経過日数/tau)とで比較する（distanceDeltaZ・intervalZの算出は
 * races[0]=前走ベースのままで変更しない）。
 *
 *   npx ts-node recencyDecayFullModelBacktest.ts
 */
...
const tauCandidates: (number | null)[] = [null, 30, 45, 60, 90, 120, 180, 365];
...
// tau===nullなら現行の順序ベース重み、それ以外なら実経過日数の指数減衰
function timeIndexScore(
  entries: HistoryEntry[], todayEpochDay: number | null, today: { trackType: string; distance: number },
  stats: Record<string, BaselineStats>, tau: number | null
): number | null {
  ...
  entries.forEach((e, slotIndex) => {
    ...
    let recency: number;
    if (tau === null) {
      recency = RECENCY_WEIGHTS[slotIndex] ?? RECENCY_WEIGHTS[RECENCY_WEIGHTS.length - 1];
    } else if (e.epochDay != null && todayEpochDay != null) {
      recency = Math.exp(-(todayEpochDay - e.epochDay) / tau);
    } else {
      recency = RECENCY_WEIGHTS[slotIndex] ?? RECENCY_WEIGHTS[RECENCY_WEIGHTS.length - 1]; // 日付不明時のフォールバック
    }
...
interface Entrant { horseName: string; finishRank: number; dateNum: number; winPayout: number | null; placePayouts: number[]; score: (tau: number | null) => number | null; }
  ...
  function buildRaces(records: RaceFactRecord[]): RaceGroup[] {
    ...
    for (const r of records) {
      ...
      race.entrants.push({
        ...
        score: (tau) => {
          const timeScore = timeIndexScore(history, epochDay, today, gradeOnlyStats, tau);
          return timeScore == null ? null : timeScore + ALPHA_JOCKEY * jz + ALPHA_INTERVAL * intervalZ - ALPHA_DISTANCE * distanceDeltaZ;
        },
      });
    }
    return [...races.values()];
...
function evaluate(races: RaceGroup[], tau: number | null): { win: Bucket; place: Bucket } {
  const win = newBucket(), place = newBucket();
  for (const race of races) {
    const scored = race.entrants
      .map(e => ({ ...e, finalScore: e.score(tau) }))
...
function main(): void {
  ...
  interface Row { tau: number | null; placeRoisEx: number[]; winRois: number[]; placeHitRates: number[]; winHitRates: number[]; }
  const rows: Row[] = [];
  for (const tau of tauCandidates) {
    const placeRoisEx: number[] = [], winRois: number[] = [], placeHitRates: number[] = [], winHitRates: number[] = [];
    for (const pr of periodRaces) {
      const { win, place } = evaluate(pr.races, tau);
      ...
      winHitRates.push(win.races > 0 ? (win.hits / win.races) * 100 : 0);
    }
    rows.push({ tau, placeRoisEx, winRois, placeHitRates, winHitRates });
  }

  console.log(`期間順: ${periods.map(p => p.label).join(' / ')}\n`);

  console.log('--- 複勝回収率(上位3件除外) ---');
  for (const r of rows) console.log(`${tauLabel(r.tau).padEnd(14)}: ${r.placeRoisEx.map(v => v.toFixed(1)).join(' / ')}  最小=${Math.min(...r.placeRoisEx).toFixed(1)}  平均=${(r.placeRoisEx.reduce((s,v)=>s+v,0)/r.placeRoisEx.length).toFixed(1)}`);

  console.log('\n--- 単勝回収率(生) ---');
  for (const r of rows) console.log(`${tauLabel(r.tau).padEnd(14)}: ${r.winRois.map(v => v.toFixed(1)).join(' / ')}  最小=${Math.min(...r.winRois).toFixed(1)}  平均=${(r.winRois.reduce((s,v)=>s+v,0)/r.winRois.length).toFixed(1)}`);

  console.log('\n--- 複勝的中率 ---');
  for (const r of rows) console.log(`${tauLabel(r.tau).padEnd(14)}: ${r.placeHitRates.map(v => v.toFixed(1)).join(' / ')}`);

  console.log('\n--- 単勝的中率 ---');
  for (const r of rows) console.log(`${tauLabel(r.tau).padEnd(14)}: ${r.winHitRates.map(v => v.toFixed(1)).join(' / ')}`);

  const base = rows.find(r => r.tau === null)!;
  console.log(`\n=== 参考: 現行(順序ベース) 複勝回収率(除外後) 最小=${Math.min(...base.placeRoisEx).toFixed(1)} 平均=${(base.placeRoisEx.reduce((s,v)=>s+v,0)/base.placeRoisEx.length).toFixed(1)} ===`);
}
